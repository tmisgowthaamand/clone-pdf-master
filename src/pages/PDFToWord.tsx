...
const PDFToWord = () => {
  ...
  const handlePythonConversion = async () => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.PDF_TO_WORD, {
        method: 'POST',
        body: formData,
      });
          ...
          <Card className="mt-8 p-4 sm:p-6 bg-gradient-to-br from-muted/50 to-muted/30 border-2 relative overflow-hidden group hover:shadow-lg transition-shadow duration-300">
            ...
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                {["📝 Convert PDF to editable Word", "🎯 Preserves formatting & layout", "⚡ Fast Python backend processing", "🔒 Secure local conversion", "💾 Supports .doc and .docx", "📊 Maintains tables and images"].map((feature, index) => (
                  <div 
                    key={index} 
